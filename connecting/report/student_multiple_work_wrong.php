...
print "ONLY_INCORRECT_ANSWERS =_" . $_REQUEST['only_incorrect_answers'] . "_";
    ...
    foreach ($this_course_data['user'] as $this_user => $this_users_data) {
      ...
      foreach ($this_users_data['quiz'] as $this_quiz => $this_problem_data) {
        ...
        foreach ($this_problem_data['problem'] as $this_problem => $this_problem_performance) {
          if (($_REQUEST['only_incorrect_answers'] == "y") && ($this_problem_performance['final_score'] == 1)) {  }
          else {
	  ...
	  for ($jj = 1; $jj < count($submission_times); $jj++) {
	    ...
	    print "<TABLE BORDER=1><TR><TD><H2>Work Part " . $jj . "</H2></TD>";
	      ...
	      print "</TD>";
            ...
            print "</TR></TABLE>";
	    print "<iframe style='width: " . $width . "; height: " . $height . ";' src='http://" . $_SERVER['SERVER_NAME'] . "/homework/session.swf?wwUserName=awangberg&problem_id=" . $session_problem_id . "&userDatabaseName=session&replayModeAsInstructor=1&overRidePaceForAdmin=300&startTime=" . $submission_times[$ii] . "&stopTime=" . ($submission_times[$jj] - 1) . "'></iframe>";
	  }
...
print "<p style='page-break-after: always'>Page $page. Student: $this_user Quiz: $this_quiz Problem: $this_problem </p>";
  ...
  print "Only Incorrect Problems: <input type='text' name='only_incorrect_answers' id='only_incorrect_answers' value='y'><BR>\n";
