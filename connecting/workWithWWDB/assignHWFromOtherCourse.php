...
if (isset($_REQUEST['CourseWithStudents']) && isset($_REQUEST['CourseWithHW']) && isset($_REQUEST['finalStartDate']) && isset($_REQUEST['finalDueDate']) && isset($_REQUEST['finalAnswerDate'])) {
  ...
  $courseWithHW = $_REQUEST['CourseWithHW'];
  $courseWithStudents = $_REQUEST['CourseWithStudents'];
  $courseWithHW = preg_replace('/\s*/', '', $courseWithHW);
...
print "        quiz was present.";
  ...
  for ($j = 0; $j < count($tests); $j++) {
    ...
    if ($quiz_is_present[$thisQuiz] == 0) {
      $query = "INSERT INTO `" . $courseWithStudents . "_set` SELECT * FROM `" . $courseWithHW . "_set` WHERE set_id='" . $thisQuiz . "'";
...
print "     error is " . mysql_error() . "<BR>";
      ...
      $query = "INSERT INTO `" . $courseWithStudents . "_problem` SELECT * FROM `" . $courseWithHW . "_problem` WHERE set_id='" . $thisQuiz . "'";
      $result = mysql_query($query, $con);
...
print "      error is " . mysql_error() . "<BR>";
      ...
      //(we also need to add "$courseWithHW/" to the start of each sourcefile.
      for ($k = 0; $k < count($conceptBanks); $k++) {

        $thisConceptBank = $conceptBanks[$k];

        $query = "INSERT INTO `" . $courseWithStudents . "_problem` SELECT * FROM `" . $courseWithHW . "_problem` WHERE set_id='" . $thisConceptBank . "'";
        $result = mysql_query($query, $con);
...
print "      error is " . mysql_error() . "<BR>";
        ...
        //now, update the source_file to make sure it points to the courseWithHW course template directory.
        $query = "UPDATE `" . $courseWithStudents . "_problem` set source_file=concat('" . $courseWithHW . "/', source_file) WHERE set_id='" . $thisConceptBank . "'";      
        $result = mysql_query($query, $con);
...
print "      error is " . mysql_error() . "<BR>";
        ...
        //also, add $thisConceptBank info to the $courseWithHW_set table:
        $query = "INSERT INTO `" . $courseWithStudents . "_set` SELECT * FROM `" . $courseWithHW . "_set` WHERE set_id='" . $thisConceptBank . "'";
        $result = mysql_query($query, $con);
...
//do we have the quiz course and the student course?
...
else if (isset($_REQUEST['CourseWithHW']) && isset($_REQUEST['CourseWithStudents'])) {
  //ask the user to select users, quizzes, and enter dates:
  print "<BR>Selected Course with students: " . $_REQUEST['CourseWithStudents'] . "\n<BR>";
  print "<BR>assigning quizzes from course: " . $_REQUEST['CourseWithHW'] . "\n<BR>";
  ...
  $userList = "";
  while($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
    $userList .= "<TR><TD><input type='checkbox' name='studentID[]' id='studentID[]' value='" . $row['user_id'] . "'checked/></TD>";
  ...
  //get the homework set from the courseWithHW course
  $set_table = $courseWithHW . "_set";
  $query = "SELECT set_id FROM `" . $set_table . "` WHERE ISNULL(assignment_type) OR assignment_type='default'";
...
<HR>
<BR>
Use this script to assign homework designed in one course to students in another course.<BR>
...
<P>
What is the course that contains the homework sets?
<select name="CourseWithHW">
<?php
  for ($i = 0; $i < count($courseList); $i++) {
    print '<option value="' . $courseList[$i] . '">' . $courseList[$i] . '</option>\n';
  }
