...
if (isset($_REQUEST['Course']) && isset($_REQUEST['practiceStartDate']) && isset($_REQUEST['practiceDueDate']) && isset($_REQUEST['practiceAnswerDate']) && isset($_REQUEST['finalStartDate']) && isset($_REQUEST['finalDueDate']) && isset($_REQUEST['finalAnswerDate'])) {
  ...
  for ($i = 0; $i < count($students); $i++) {
    for ($j = 0; $j < count($tests); $j++) {
      ...
      $thisTest = $tests[$j];
      $resultOfPhp = `php /var/www/html/connecting/workWithWWDB/createPracticeAndFinalQuizBasedOnConcepts.php $course $thisStudent $thisTest $practiceStartDate $practiceDueDate $practiceAnswerDate $finalStartDate $finalDueDate $finalAnswerDate`;
      print "php#createPracticeAndFinalQuizBasedOnConcepts.php#$course#$thisStudent#$thisTest#$practiceStartDate#$practiceDueDate#$practiceAnswerDate#$finalStartDate#$finalDueDate#$finalAnswerDate#.....><PRE>$resultOfPhp</PRE><BR>";
    }
  }
}
