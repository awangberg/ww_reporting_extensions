...
if ($argc != 10) {
  die("Usage: createPracticeAndFinalQuizBasedOnConcepts.php <courseName> <studentID> <quizName> <startPracticeDate> <endPracticeDate> <answerPracticeDate> <startFinalQuizDate> <endFinalQuizDate> <answerFinalQuizDate>");
}
...
//get the list of conceptBanks that are appropriate to assign for each quiz problem on the pcb:
$list_of_assignable_practice_conceptBanks = array();
$tmp_list_of_assignable_practice_conceptBanks = explode("<BR>", `php /var/www/html/connecting/workWithWWDB/list_practice_problems_for_pcb_quiz.php`);
foreach ($tmp_list_of_assignable_practice_conceptBanks as $k => $v) {
  list($tmp_cb, $tmp_n, $tmp_assign_cb, $tmp_rating, $tmp_sp1, $tmp_pct, $tmp_sp2, $tmp_last_par) = explode(" ", $v);
  $list_of_assignable_practice_conceptBanks[$tmp_cb][$tmp_assign_cb] = $tmp_assign_cb;
}
...
for ($i = 0; $i < count($practiceConceptNumbers); $i++) {
  ...
  foreach ($list_of_assignable_practice_conceptBanks[$conceptBank] as $k => $data) {
    $conceptBankForAllPracticeProblems[] = $data;

    //$tmp_string = `php /var/www/html/connecting/workWithWWDB/get_n_ValidProblemsFromConceptBank.php 1 $courseName $data`;
    $tmp_string = `php /var/www/html/connecting/workWithWWDB/get_n_ValidProblemsFromConceptBank.php 1 SessionExperiment $data`;

    $practiceProblems[] = $tmp_string;
    print "Added $tmp_string to $practiceProblems, through call to turn_n_ProblemsFromConceptBank.php 1 SessionExperiment $data\n";
  }

  //$conceptBankForAllPracticeProblems = array_merge($conceptBankForAllPracticeProblems, $list_of_assignable_practice_conceptBanks[$conceptBank]);
...
for ($i = 0; $i < count($practiceProblems); $i++) {
  ...
  $query .= " VALUES('$practiceSet', $tmpi, 'SessionExperiment/$sourceFile', 1, -1)";
  $result = mysql_query($query, $con);
  ...
  print "php /var/www/html/connecting/workWithWWDB/recordConceptBankFor_courseID_userID_practiceSet_problemID_pgSourcefile.php " . $conceptBankForAllPracticeProblems[$i] . " # " . $courseName . " # " . $studentID . " # " . $practiceSet . " # " . $tmpi . " # " . $sourceFile . "\n";
